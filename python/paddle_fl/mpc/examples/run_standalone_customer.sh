...
# Running party 1 with /usr/bin/python3 falcon/train_fc_sigmoid.py 1 127.0.0.7 6379
echo "Begin to run ......."
for role in {1..2}; do
    $PYTHON $SCRIPT --role=$role 2>&1 >/dev/null &
done

# for party of role 0, run in a foreground mode and show the output
echo  $PYTHON $SCRIPT 0
$PYTHON $SCRIPT --role=0
